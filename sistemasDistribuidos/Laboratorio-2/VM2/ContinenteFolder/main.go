...
import (
	...
	pb "ContinenteFolder/proto" // Asegúrate de que sea el path correcto generado a partir del .proto

	"google.golang.org/grpc"
)

const (
	port        = ":8085"            // Puerto donde escucha el Data Node
...
func (s *server) GetDigimonStatus(ctx context.Context, req *pb.DigimonRequest) (*pb.DigimonResponse, error) {
	if req.Opt == "-1" {
		fmt.Println("Apagando ContinenteFolder")
		os.Exit(0)
	}
