...
import (
	...
	pb "ContinenteServer/proto" // Asegúrate de que sea el path correcto generado a partir del .proto

	"google.golang.org/grpc"
)

const (
	port        = ":8083"            // Puerto donde escucha el Data Node
...
func (s *server) GetDigimonStatus(ctx context.Context, req *pb.DigimonRequest) (*pb.DigimonResponse, error) {
	if req.Opt == "-1" {
		fmt.Println("Apagando ContinenteServer")
		os.Exit(0)
	}
...
func seleccionarDigimonAleatorio(digimons []string, ps float64) string {
	...
	// fmt.Printf("Resultado de selección: %s\n", resultado)
	return resultado
}
